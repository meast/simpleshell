...
if [ ! -x "${APKToolPath}" ]; then
    echo "${APKToolPath} is not executable..."
    exit
fi
if [ ! -x "${Dex2JARPath}" ]; then
    echo "${Dex2JARPath} is not executable..."
    exit
fi
...
SaveInflatDir=${FilePath}/${APKFileName}_inflat
...
for DEXFile in ${ArrDEXFiles}; do
    ...
    # dex2jar
    ${Dex2JARPath} "${SaveDEXDir}/${DEXFileName}" -f -o "${SaveJARDir}/${APKFileName}_${DEXFileName%'.dex'}_dex2jar.jar"
done
