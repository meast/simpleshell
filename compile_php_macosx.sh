...
if [ -x $SrcDir ] && [ -w $DesPDir ]; then
    ...
    if [ ! -f $DesDir/bin/php ] && [ ! -f $desDir/bin/phpize ] && [ ! -f $DesDir/bin/php-config ]; then
        ...
        if [ -f $ApxsPath ] && [ "$WithApxs" == "$ApxsPath" ]; then
            echo "with apxs2"
            sudo ./configure --prefix=$DesDir --enable-cli --enable-cgi --enable-fpm --enable-mysqlnd --with-apxs2=$ApxsPath --with-iconv-dir=/usr/local/opt/libiconv
        else
            echo "without apxs2"
            sudo ./configure --prefix=$DesDir --enable-cli --enable-cgi --enable-fpm --enable-mysqlnd --with-iconv-dir=/usr/local/opt/libiconv
        fi
        if [ ! -f $SrcDir/Makefile ]; then
